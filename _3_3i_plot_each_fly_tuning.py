...
ROI = 'ROI_1'
stim_l = ['2p_400']
compart_l = ['somaL', 'somaR', 'somaL_ctrl', 'somaR_ctrl']
        ...
        for fly_path in glob(date_path+'/*'):
            ...
            for i, stim in enumerate(stim_l):
                for j, trial_i in enumerate(trial_l):
                    trial = compart + '_' + stim + trial_i
                    trial_glob = glob(fly_path + '/*' + trial)
                    if trial_glob:
                        trial_path = trial_glob[0]
                        single_stim_df = merge_all_trials(trial_path, imaging_rate)
                        if 'MEC' in compart or 'PTX' in compart:
                            color = 'r'
                        else:
                            color = 'k'
                        sns.lineplot(data=single_stim_df, x='time', y=ROI, label=stim+'_trial_'+str(j+1), errorbar='se', color=color, alpha=alpha_l[j])
                plot_PhotoStim(ax, imaging_rate, merged=True)

                # plt.show()
                fig_path = os.path.join(each_fly_path, '%s_%s_Across_trial.png' % (fly, compart))
                plt.savefig(fig_path, dpi=600)
                plt.close()
                print(fig_path)
